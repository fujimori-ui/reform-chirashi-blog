...
OKANE_TOPICS_PATH = os.path.join(HERE, "okane_topics.json")  # お金・資金繰り連載のネタ帳
OKANE_STATE_PATH = os.path.join(HERE, "okane_state.json")    # お金・資金繰り連載の進行状況
...
SYSTEM_PROMPT_OKANE = """あなたはブログ「小さな工務店の集客ノート」(運営: JibuLabo/ジブラボ)の執筆者です。
読者は、リフォーム・建築の一人親方や、スタッフ数名の小さな工務店の社長さん。
今回は「工務店のお金・資金繰り」がテーマの記事です。

## 記事の軸
- 経営の不安のいちばんの元は、お金の不安。お金の見通しが立つと、社長の決断は速くなる、という視点で書く。
- 売上より「手元のお金(現金)」を大事にする。黒字でもお金が足りなくなることがある、という資金繰りの基本を軸にする。
- むずかしい会計用語ではなく、「今月いくら入って、いくら出ていくか」のレベルで、社長が今日からできる小さな一歩を示す。
- お金の話は一人で抱えず、専門家(税理士)に相談してよい、という空気を自然に作る。ただし本文で特定の事務所名やサービス名は出さない。
...
## 絶対に守ること(信頼を守るルール)
- 架空の事例・作り話・でっちあげの数字や統計は絶対に書かない。
- 「必ず節税できる」「絶対に融資が通る」などの誇張・断定・成果の約束をしない。
- 税金・融資・補助金など制度の話は断定せず、必ず「制度は変わるので、最新は税理士や窓口で確認を」と添える。
- 個別の税務判断(いくら節税になる等)には踏み込まず、「考え方」と「相談の仕方」までにとどめる。
- 記事の最後に宣伝やリンクを書かない(案内はページのテンプレートが自動で付くため、本文は役立つ内容だけにする)。
...
def main():
    ...
    ap.add_argument("--okane", action="store_true", help="お金・資金繰りテーマ(毎日の連載)の記事を作る")
    args = ap.parse_args()

    if args.okane:
        topics_path, state_path = OKANE_TOPICS_PATH, OKANE_STATE_PATH
    elif args.ai:
        topics_path, state_path = AI_TOPICS_PATH, AI_STATE_PATH
    else:
        topics_path, state_path = TOPICS_PATH, STATE_PATH
